...
从软件开发的角度来看，「非检查型（*unchecked*）」异常和「检查型（*checked*）异常」是比较实用的分类方式。非检查型异常在一定程度（比如不考虑 `java.lang.Error` 这种极端情况）上也可以认为等价于「运行时异常」（`java.lang.RuntimeException`）；相应地，检查型异常也可以称为「编译时异常」。二者最明显的差距就在于，==前者允许编码阶段不提供异常处理，但是后者必须提供。==
